...
from .utils import summinexp_vmap, log1mexp
...
def compute_partition_function(eps, N, beta):
  ...
  def outer_loop(M, Q):
    ...
    Q= Q.at[M-1].set(C[1]*lax.fori_loop(1, M, inner_recursion, 1.)/M)
    return Q
...
compute_aux_partition_function_vmap = jit(vmap(compute_aux_partition_function, in_axes = (0, None, None), out_axes=(1)))
...
def compute_correlations(n, eps, beta):
  ...
  correlations = (jnp.outer(n*jnp.exp(beta*eps), 1) - jnp.outer(1, n*jnp.exp(beta*eps))) / (jnp.outer(jnp.exp(beta*eps),1) - jnp.outer(1, jnp.exp(beta*eps)))
...
def compute_correlations_degen(eps, Q, beta):
  ...
  correlations_degen = expeps2*lax.fori_loop(0, N-2, inner_recursion, 1.)/(Q[N-1]*Q[N-2])
...
compute_correlations_degen_vmap = jit(vmap(lambda eps, Q, beta: compute_correlations_degen((eps[0]+eps[1])/2, Q, beta), in_axes=(1, None, None), out_axes=0))

def sinkhorn(n, N, beta=1., eps = None, max_iters=100, threshold = 10**-10, old=False, comp_correlations=False, degen_cutoff=10**-7, verbose=True):
  ...
  if comp_correlations:
    ...
    degens = jnp.where(jnp.abs(jnp.ones((norb, norb))*jnp.exp(beta*eps)-(jnp.ones((norb, norb))*jnp.exp(beta*eps)).T+jnp.eye(norb)) < degen_cutoff)
    if degens[0].size > 0:
      correlations = correlations.at[degens[0], degens[1]].set(compute_correlations_degen_vmap(jnp.vstack((eps[degens[0]], eps[degens[1]])), Q, beta).block_until_ready())
