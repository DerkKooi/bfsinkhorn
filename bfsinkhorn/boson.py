...
def compute_free_energy(eps, N, beta):
  ...
  def inner_loop(i, F):
    exponents = lax.dynamic_slice(D, [i-1], [N]) \
              + F[1:N+1]
...
compute_aux_free_energy_vmap = jit(vmap(compute_aux_free_energy, in_axes=(0, None, None), out_axes=0))
...
def compute_correlations(n, eps, F, beta):
  ...
  k = jnp.arange(1,N+1)
  correlations_diag = vmap(lambda eps: jnp.sum((2*k-1)*jnp.exp(-beta*(jnp.flip(F[:N])+eps*k-F[N]))), in_axes=(0), out_axes=0)(eps)
...
compute_correlations_degen_vmap = jit(vmap(lambda eps, F, beta: compute_correlations_degen((eps[0]+eps[1])/2, F, beta), in_axes=(1, None, None), out_axes=0))
...
def sinkhorn(n, N, beta=1., eps = None, max_iters=100, threshold = 10**-10, old=False, comp_correlations=False, degen_cutoff=10**-7, verbose=True):
  ...
  if comp_correlations:
    ...
    if degens[0].size > 0:
      correlations = correlations.at[degens[0], degens[1]].set(compute_correlations_degen_vmap(jnp.vstack((eps[degens[0]], eps[degens[1]])), F, beta).block_until_ready())
