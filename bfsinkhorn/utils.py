...
def log1mexp(a):
    ...
    '''
    return jnp.heaviside(a-jnp.log(2), 0.5)*jnp.log1p(-jnp.exp(-a))+(1-jnp.heaviside(a-jnp.log(2), 0.5))*jnp.log(-jnp.expm1(-a))
...
minlogsumminexp_vmap = jit(vmap(lambda k, exponents: minlogsumminexp(k*exponents), in_axes=(0, None), out_axes=0))

#vmap the sum of exponents for different k multiplying the exponents
summinexp_vmap = jit(vmap(lambda k, exponents: jnp.sum(jnp.exp(-k*exponents)), in_axes=(0, None), out_axes=0))
